...
function Song.new(file)
	...
	local self = setmetatable({
		...
		Timebase = score[1];
		IsPlaying = false;
		Gui = script.Parent.Screen.SurfaceGui;
...
function Song:_log(event)	
	if #self.Gui:GetChildren() - 2 > self._maxLogs then
		for i,v in self.Gui:GetChildren() do
			if v.Name == 'Log' then
				game.Debris:AddItem(v)
			end
		end
	end
	
	local clone = self.Gui.TextLabel:Clone()
	clone.Text = table.concat(event,' ')
	clone.Visible = true
	clone.Name = 'Log'
