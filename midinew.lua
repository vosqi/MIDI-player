local Midi = Instance.new("Part")
Midi.Name = "Midi"
Midi.BottomSurface = Enum.SurfaceType.Smooth
Midi.TopSurface = Enum.SurfaceType.Smooth
Midi.Color = Color3.fromRGB(17, 17, 17)
Midi.Size = Vector3.new(18, 12, 1)
Midi.CFrame = CFrame.new(-4, 6.000001907348633, -21.5)
...
SurfaceGui.PixelsPerStud = 300
SurfaceGui.Parent = Midi
...
TextLabel.TextDirection = Enum.TextDirection.LeftToRight
...
UIListLayout.Parent = SurfaceGui

Midi.Anchored = true
Midi.CFrame = owner.Character.PrimaryPart.CFrame * CFrame.new(0,1.5,-5)
Midi.Parent = script

local midiconsole = {}
midiconsole.__index = midiconsole

AlignColumns = function(strings, width)
	width = width or 100
...
midiconsole.new = function()
	local self = setmetatable({},midiconsole)
	self.gui = Midi.SurfaceGui
...
function midiconsole:log(...)
	local t = AlignColumns({...})
	local label = self.textlabel:Clone()
...
M.Number2patch = {   -- General MIDI patch numbers:
	[0]='Acoustic Grand',
	[1]='Bright Acoustic',
	[2]='Electric Grand',
	[3]='Honky-Tonk',
...
M.Notenum2percussion = {   -- General MIDI Percussion (on Channel 9):
	[33]='Metronome Click',
	[34]='Metronome Bell',
...
local midiconsole = midiconsole.new()
...
function Song.new(file, par)
	local succ,midiData = pcall(function()
		return HttpService:GetAsync(file)
	end)

	if succ == false then
		return succ,midiData
	end
...
function getNoteFromNotenum(notenum)
    local octave = math.floor(notenum / 12) - 1
    local notes = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"}
    local noteIndex = (notenum % 12) + 1
    local note = notes[noteIndex]

    return note, octave
end

local function playNote(note, time, volume, instrument, parent, event)
	midiconsole:log(tostring(event[5]),getNoteFromNotenum(note),volume*(defaultvol*vol),event[4], event[1])
...
function Song:_parse(event)
	...
	elseif (eventName == "note") then
		...
		if(event[4]==9) then
			...
			local audio = Instance.new("Sound", self._parent)-- Create the audio
			audio.SoundId = Percussion[tostring(event[5])] or ''
			audio.Pitch = 1
			audio.Volume = (event[6]/127)*(defaultvol*vol)

			midiconsole:log(tostring(event[5]),1,(event[6]/127)*(defaultvol*vol),event[4], event[1])
		...
		elseif event[4] == 9 then
			playNote(event[5]-35,event[3] / self.Timebase,event[6]/127, instruments['Baritone'], self._parent, event)
		else
			playNote(event[5]-35,event[3] / self.Timebase,event[6]/127, instruments['Piano'], self._parent, event)
		end
	end
end
...
owner.Chatted:Connect(function(msg)
	if msg:sub(1,1) == '/' then
		...
		if cmd[1] == '/play' then
			...
			if cmd[2] == 'song' then
				succ,res = Song.new('https://vxsqi.me/scripts/midi/song.mid',owner.Character.Head)
			else
				succ,res =  Song.new(cmd[2],owner.Character.Head)
			end
